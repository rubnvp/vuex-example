...
const getters = {
    ...
    rankings(state, getters) {
        const rankingObject = getters.pokemonsList
            .reduce((accum, pokemon) => {
        ...
        return Object.values(rankingObject)
            .sort((rankingLeft, rankingRight) => rankingLeft.value > rankingRight.value ? -1 : 1);
    },
};
...
const actions = {
    ...
    addPokemon({state, commit}, {type, name}) {
        if (name && !state.pokemons[name]) { // add only if not exists
    ...
    attackPokemon({state, commit, dispatch}, {src, target}) {
        ...
        if (src.type === POKEMON_TYPE.FIRE && target.type === POKEMON_TYPE.PLANT ||
    ...
    removePokemon({state, commit}, pokemon) {
        ...
        setTimeout(() => {
            if (state.pokemons[pokemon.name] === pokemon) { // check if the pokemon is the same
